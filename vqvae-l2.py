...
def train_wavelet_vqvae(model, dataloader, optimizer, epochs=10, device="cuda"):
    ...
    """
    writer = SummaryWriter(log_dir="runs/wavelet_vqvae_l2_ssim")
    model.to(device)
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(
            ...
            wavelet_recon_loss = -ssim(reconstructed, data)
            writer.add_scalar("Wavelet Recon Loss/train", wavelet_recon_loss.item(), batch_idx)
