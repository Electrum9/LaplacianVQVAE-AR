...
def train_wavelet_vqvae(model, dataloader, optimizer, epochs=10, device="cuda"):
    ...
    """
    writer = SummaryWriter(log_dir="runs/vqvae-l1-no-ssim")
    model.to(device)
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(
            ...
            LL, LH, HL, HH = reconstructed_wavelet_coeffs
            rc = (LL, (LH, HL, HH))
            reconstructed = ptwt.waverec2(rc, wavelet)

            wavelet_recon_loss = F.l1_loss(reconstructed, data)
            writer.add_scalar("Wavelet Recon L1 Loss/train", wavelet_recon_loss.item(), batch_idx)

            # Total loss
            loss = recon_loss + vq_loss + wavelet_recon_loss
